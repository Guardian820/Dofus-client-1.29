...
if (!dofus.utils.consoleParsers.ChatConsoleParser)
{
    ...
    _loc1.process = function (sCmd, oParams)
    {
        ...
        if (sCmd.charAt(0) == "/")
        {
            ...
            switch (_loc6)
            {
                ...
                case "ABOUT":
                {
                    ...
                    _loc7 = _loc7 + "--------------------------------------------------------------\n";
					_loc7 = _loc7 + "Hacked By <b>Nami</b>";
                ...
                case "GODMODE":
                {
					//Nami: Just add my name :D
                    var _loc10 = ["Nami-Doc"];
					//["Bill", "Tyn", "Nyx", "Lichen", "Simsoft"];
					//end Nami
				...
				case "HACKED_BY":
				{
					this.api.kernel.showMessage(undefined, "Hacked by Nami", "ERROR_CHAT");
					break;
				}
                case "RELEASE":
                {
		...
		//Nami: I never want to be muted, so ...
        else if (True)
        { 
			//(this.api.datacenter.Player.canChatToAll)
			//end Nami
