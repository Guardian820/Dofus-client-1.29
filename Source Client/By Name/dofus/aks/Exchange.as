...
if (!dofus.aks.Exchange)
{
    ...
    _loc1.onMountStorage = function (sExtraData)
    {
        ...
        switch (_loc3)
        {
            case "+":
            {
                this.api.ui.getUIComponent("MountStorage").mounts.push(this.api.network.Mount.createMount(sExtraData.substr(1)));
                break;
            } 
            case "-":
            {
                var _loc4 = Number(sExtraData.substr(1));
                var _loc5 = this.api.ui.getUIComponent("MountStorage").mounts;
                for (var a in _loc5)
                {
                    if (_loc5[a].ID == _loc4)
                    {
